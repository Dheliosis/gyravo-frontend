...
<nav aria-label="Breadcrumb">
  <ol role="list" class="flex items-center space-x-1 text-sm text-gray-500">
    <li
			v-for="(el, value) in list"
			:key="el.name"
			class="flex items-center"
		>
      <a class="block transition-colors hover:text-gray-700" :href="el.link"> {{el.name}} </a>
			<svg v-if="value !== list.length -1" xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" viewBox="0 0 20 20" fill="currentColor">
